...
export const Header = () => {
  ...
  return <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/95 backdrop-blur-md shadow-md py-3' : 'bg-white/80 backdrop-blur-sm py-5'}`}>
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            ...
            to: "/clients",
            label: "Clients"
          }].map(item => <Link key={item.label} to={item.to} className={`font-medium transition-colors duration-300 relative text-gray-800 hover:text-orange-500 after:content-[''] after:absolute after:w-full after:scale-x-0 after:h-0.5 after:bottom-0 after:left-0 after:origin-bottom-right after:transition-transform after:duration-300 hover:after:scale-x-100 hover:after:origin-bottom-left after:bg-orange-500 ${location.pathname === item.to ? 'text-orange-500 after:scale-x-100 after:bg-orange-500' : ''}`}>
                {item.label}
              </Link>)}
          </nav>

          <div className="flex items-center">
            <Button className="bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 shadow-md hover:shadow-lg transition-all duration-300 btn-hover-shine rounded-xl text-white">
              <Link to="/contact">Contact Us</Link>
            </Button>
            
            <button className="md:hidden ml-4 p-2 hover:bg-gray-100/10 rounded-lg transition-colors text-gray-800" onClick={() => setIsMenuOpen(!isMenuOpen)} aria-label="Toggle menu" aria-expanded={isMenuOpen}>
