...
const Clients = () => {
  const [testimonialApi, setTestimonialApi] = useState(null);
  const [clientLogoApi, setClientLogoApi] = useState(null);
  const isMobile = useIsMobile();

  useEffect(() => {
    const interval = setInterval(() => testimonialApi?.scrollNext(), 5000);
    return () => clearInterval(interval);
  }, [testimonialApi]);

  useEffect(() => {
    const interval = setInterval(() => clientLogoApi?.scrollNext(), 3000);
    return () => clearInterval(interval);
  }, [clientLogoApi]);
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="pt-32 pb-16 bg-gradient-to-br from-gray-900 via-[#333333] to-gray-800 text-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10 text-center">
          ...
          <p className="text-xl text-gray-300 mb-8">
            Our client portfolio spans across various industries, especially freight forwarding and logistics.
          </p>
        </div>
      </section>

      <section className="py-16 bg-gray-50 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-[#F58220]/5 rounded-full blur-3xl" />
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-[#FFC78E]/5 rounded-full blur-3xl" />

        <div className="container mx-auto px-4 relative z-10">
          <h2 className="font-heading font-bold text-3xl md:text-4xl text-center mb-12 text-gray-800">Our Clients</h2>
          <Carousel setApi={setClientLogoApi} opts={{ align: "start", loop: true }}>
            <CarouselContent>
              {clientLogos.map((client, idx) => (
                <CarouselItem key={idx} className={`${isMobile ? "basis-1/2" : "basis-1/4"} p-2`}>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          ...
          <Carousel setApi={setTestimonialApi} opts={{ align: "start", loop: true }}>
            <CarouselContent>
              {testimonials.map((t, idx) => (
                <CarouselItem key={idx} className={isMobile ? "basis-full" : "basis-1/2"}>
                  <div className="p-4">
                    <Card className="border-none shadow-lg hover:shadow-xl transition-all duration-300 h-full overflow-hidden">
                      <CardContent className="p-0">
                        <div className="bg-gradient-to-br from-[#F58220]/10 to-white p-8 flex flex-col h-full relative">
                          <div className="text-6xl text-[#F58220]/20 absolute top-4 right-4">"</div>
                          <blockquote className="text-gray-700 italic mb-6 relative z-10 flex-grow">{t.quote}</blockquote>
                          <div>
      ...
      <section className="py-16 bg-gradient-to-r from-[#2C2C2C] to-[#444444] text-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10 text-center">
          <h2 className="font-heading font-bold text-3xl md:text-4xl mb-6 bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">
